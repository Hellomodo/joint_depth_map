...
class HourglassNet3D(nn.Module):
  ...
  def forward(self, image, hm):
    ...
    for i in range(self.nStack):
      ...
      depMap.append(tmpOut)

      tmpDep = torch.sum( torch.sum(torch.mul(tmpOut, hm), dim=2), dim=2)
      depth.append(tmpDep)
...
class DepthLossFunc(nn.Module):

  def __init__(self, beta):
    super(DepthLossFunc, self).__init__()
    self.beta = beta
    return

  def forward(self, depMap, gt2dMap, gtDep, beta):
    #dep = torch.sum(depMap.mul_(gt2dMap),[1,2])
    #loss = torch.sum(torch.sudep)

    dep = torch.sum(depMap.mul_(gt2dMap), [1, 2])
    matched = torch.pow(func.pairwise_distance(dep,gtDep),2)
    loss = torch.mean(matched)
    #loss = torch.sum((dep-gtDep)**2)
    return loss
